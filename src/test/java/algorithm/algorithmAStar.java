...
/**
 * Tests if AStar search works correctly
 */
public class algorithmAStar {
    ...
    @Test
    public void testAStarHamming() {
        for (int i = 0; i < puzzles.size() - 2; i++) {
            Result result = AStar.solve(new Board(puzzles.get(i)), new HammingDistance(), TimeUnit.NS, DebugMode.ON);
    ...
    @Test
    public void testAStarLinearConflict() {
        for (int i = 0; i < puzzles.size(); i++) {
            Result result = AStar.solve(new Board(puzzles.get(i)), new ManhattanDistance(), TimeUnit.NS, DebugMode.ON);
            Assertions.assertEquals(depth.get(i), result.getDepth());
        }
